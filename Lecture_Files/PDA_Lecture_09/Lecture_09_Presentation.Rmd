...
We already saw tibbles and how to convert data frames into tibbles (using `as.tibble()`). 
...
```{r eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE, na.rm=FALSE}
  # Load the package RCurl
  library(RCurl)
  # Import the HANES data set from GitHub; break the string into two for readability
  # (Please note this readability aspect very carefully)
...
All data sets are stored with encoding. Each hexadecimal number represents a byte of information: 48 is H, 61 is a, and so on. 

The mapping from hexadecimal number to character is called the encoding, and in the case of english the standard of encoding is ASCII. 

The function `charToRaw()` will list the encoding of characters in the ASCII format. 
...
However, we need encodings for other languages and UTF-8 is the standard that can encode just about every character used by humans today, as well as many extra symbols (like emoji!). 

We can parse such encodings (that will appear gibberish if we read it in the english system) through `parse_charecter()` function:
...
<br>
<span style="color:blue">**Classwork/Homework**</span>: Read section 11.4 and 11.5 (Parsing a file and Writing to a file sections ) in R for Data Science book.
...
Another approach is to use the helpers provided by the library _lubridate_. 
...
We can make use of _time spans_. 
...
time difference can be little painful to work with when we are interested in durations. 
...
For example, consider MIMIC3 admissions table. There are two variables - `edregtime` and `edouttime` (emergency department registration time and emergency department out time, respectively) that represents one variable: `time stamp in emergency`, a larger class of variable. 
...
For example, in the above data set we can _unite_ hospital admission id `hadm_id` and the admission type `admission_type` to tie a reason why the patient got admitted. By default the unite will use underscore. 
...
The data comes from the 2014 World Health Organization Global Tuberculosis Report, which can be downloaded from [WHO TB Data](http://www.who.int/tb/country/data/download/en/). 

The data is a part of _tidyverse_ package, so you don't have to download it.
...
Combining data in multiple tables is an imporant part of the wrangling process. Usually these combinations are facilitated by _joins_. 
...
2. Having said the above, it is also dependent on the question of interest. 
...
3. Check that none of the variables in the primary key are missing. 

If a value is missing then it can’t identify an observation!

4. Check if the foreign keys match primary keys in another table. 

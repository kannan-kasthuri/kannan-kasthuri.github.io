...
We saw that there is a good correlation between LDLESTIMATE and CHOLESTEROLTOTAL suggesting there is possibly a linear relationship between them. Assuming there is a linear relationship between them, is it possible to predict the values of CHOLESTEROLTOTAL when we have the values for LDLESTIMATE?  When the outcome (random) variable $Y$ has a _linear_ relationship with one or more predictor variables $X$, we can use linear regression to predict $Y$ from $X$.   

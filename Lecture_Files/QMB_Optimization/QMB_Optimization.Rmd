...
Thus, we have these two _hidden variables_ $\theta_{A1C}$ and $\theta_{GLU}$ that can dermine the probability of someone being diabetic. We have to figure these out from the observations. This can be done as follows:
...
```{r eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
  ...
  # cost = sum(likelihood)
  # return(cost)
  return(predicted.miss.from.hung[2])
  }
  ...
  DB.MG.cost = function(params, data=DB.obs) {
    ...
    predicted.diab = predicted.diab.A1C + predicted.diab.GLU - (predicted.diab.A1C * predicted.diab.GLU)
    
    likelihood = ifelse(data$DX_DBTS_L, predicted.diab, 1 - predicted.diab)
    cost.DB = sum(likelihood) #-sum(log(likelihood)) 
    # return(cost.DB)
    return(predicted.diab.GLU[2])
  }
```
...
# DB.grid$cost = mapply(function(w,h){DB.MG.cost(c(w,h))}, 

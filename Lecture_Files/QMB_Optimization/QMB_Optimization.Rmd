...
Thus, we have these two _hidden variables_ $\theta_{A1C}$ and $\theta_{GLU}$ that can dermine the probability of someone being diabetic. We have to figure these out from the observations. Thus we first extract this data from the data table:
...
```{r eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
  ...
  # DB.obs <-  data.frame(A1C_var = rbinom(DB.n.obs,1,.6) == 1,
  #                            GLUCOSE_var = rbinom(DB.n.obs,1,.4) == 1)
  # DB.obs$A1C_L <- DB.dataframe$A1C_var
  # DB.obs$GLUCOSE_L <- DB.dataframe$GLUCOSE_var
...
# These will be revealed later, but just trust that they exist for now
  ...
  missing.work.prob = ifelse(attendance.obs$Working,0.24, 0)
  missing.hung.prob = ifelse(attendance.obs$Hungover, 0.42, 0)
  attendance.obs$Missing = rbinom(n.obs,1,missing.work.prob) | rbinom(n.obs,1,missing.hung.prob)
  
  tmp = select(HANES, A1C, GLUCOSE, DX_DBTS) %>% filter(A1C >= 7 & GLUCOSE >= 140) %>% mutate(DX_DBTS_L = as.logical(ifelse(DX_DBTS == "DIAB",1,0))) %>% select(DX_DBTS_L)
  
  attendance.obs$Missing <- unlist(tmp)
  
  attendance.obs

  # DB.obs$DX_DBTS_L <- rbinom(DB.n.obs,1,missing.work.prob) | rbinom(DB.n.obs,1,missing.hung.prob)
...
We then define our likelihood function. This function takes the parameters in a single argument and returns the cost vector as negative log likelyhood function.
...
```{r eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
  ...
  # print(predicted.miss.from.work)
  ...
  predicted.miss = predicted.miss.from.work + predicted.miss.from.hung - 
    (predicted.miss.from.work * predicted.miss.from.hung) + 0.00001
  ...
  # cat(likelihood-predicted.miss, "\n")

    # print(predicted.miss)
  cost = -sum(log(likelihood))
  # cost = sum(likelihood)
  return(cost)
  # return(predicted.miss[2])
  }
  ...
  DB.MG.cost = function(params, data=DB.obs) {
    ...
    predicted.diab = predicted.diab.A1C + predicted.diab.GLU - (predicted.diab.A1C * predicted.diab.GLU) + 0.000000001
    
    likelihood = ifelse(data$DX_DBTS_L, predicted.diab, 1-predicted.diab)
    # print(likelihood)
    cost.DB = -sum(log(likelihood)) 
    return(cost.DB)
    # return(predicted.diab[2])
  }
```
...
# test <- mapply(function(w,h){DB.MG.cost(c(w,h))}, 
#                               DB.grid$A1C, DB.grid$GLU)
# test

DB.plot = ggplot(DB.grid, aes(x=A1C, y=GLU)) +
  geom_raster(aes(fill=cost.DB)) + scale_fill_gradient(low='red', high='yellow', trans = 'log') +
  stat_contour(aes(z=cost.DB),binwidth=10, color = 'black', linetype = 'dashed') +
  xlab('P(Missing | Working)') + ylab('P(Missing | Hungover)')
print(DB.plot)

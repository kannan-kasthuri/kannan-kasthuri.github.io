...
### Data science fundamentals 01: Visualize and explore  (`r format(Sys.Date(), "%b %d, %y")`)

---

Data exploration is the art of looking at the data, rapidly generating hypotheses, quickly testing them, then repeating again, and again, and again. 

The goal of data exploration is to generate many promising leads that we can later explore in more depth. 

Visualization is key for data exploration, but not sufficient. 

We need data transformation and exploratory data analysis which we will look into later (next couple of classes). 

Modeling is important too but we will explore it later.
...
#### The Graphics package _ggplot2_

Packages are extentions of R's basic features  


We load a pakage using the `library()` or `require()`
...
With ggplot2, we generate a graphics object using the function `ggplot()`. `ggplot()` creates a coordinate system that we can add layers to. 

The first argument of `ggplot()` is the dataset to use in the graph. So `ggplot(data = HANES)` creates an empty graph.

Next, we add one or more layers to `ggplot()`. 

##

We create a `ggplot` object as follows:
...
This defines how variables in our dataset are mapped to visual properties. 
...
ggplot2 looks for the mapped variable in the data argument, in this case, HANES.
...
Aesthetics include things like the **size**, the **shape**, or the **color** of the points. 

We can display a point in different ways by changing the values (the keyword) of its aesthetic properties.    
  
Aesthetics / Scales:

```{r out.width = "50%", echo=FALSE}
library(png)
knitr::include_graphics("scalesExample.png")
```


##

We can convey information about the data by mapping the aesthetics in the plot to the variables in HANES dataset.  

For example, we can map the colors of our points to the DX_DBTS variable to reveal the diabetes status of each person.
...
ggplot2 will automatically assign a unique level of the aesthetic (here a unique color) to each unique value of the variable, a process known as _scaling_. 
...
In the above example, we mapped DX_DBTS to the color aesthetic, but we could have mapped AGEGROUP to the size aesthetic in the same way. 

In this case, the exact size of each point would reveal its AGEGROUP affiliation. 

We get a warning here, because mapping an unordered variable (AGEGROUP) to an ordered aesthetic (size) is not a good idea.
...
For each aesthetic, we use `aes()` to associate the name of the aesthetic with a variable to display. 
...
The syntax highlights a useful insight about x and y: the x and y locations of a point are themselves aesthetics, visual properties that we can map to variables to display information about the data.

##

Once we map an aesthetic, ggplot2 takes care of the rest. 
...
We can also set the aesthetic properties of our geom manually. For example, we can make all of the points in our plot blue:
...
The first argument of `facet_wrap()` should be a formula, which we create with **~** sign followed by a variable name (here “formula” is the name of a data structure in R, not a synonym for “equation”). 
...
We see that both males and females have almost the same distribution. However, plotting the facet grid with AGEGROUP vs DX_DBTS,
...
reveals, there are hardly people with diabetes in the age group 20-39, while there are many more people who get diabetes as they age.

This is an example of **directed insight** obtained through visualization. 
...
```{r eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.height=3.5}
...
```{r eval=TRUE, warning=FALSE, message=FALSE, echo=FALSE, fig.height=3.5}
...
In ggplot2 syntax, they're called **geoms**.
...
The plot on top uses the `geom_point`, and the plot in the bottom uses the `geom_smooth`, a smooth line fitted to the data.
...
```{r eval=FALSE, warning=FALSE, message=FALSE, echo=TRUE, results="hide"}
  ...
  ggplot(data = HANES) + 
  geom_point(mapping = aes(x = UCREATININE, y = UALBUMIN, 
                           color = GENDER)) + 
  xlim(c(0,100)) + ylim(c(0,10))
```


And here is the smooth (line) geom code:

```{r eval=FALSE, warning=FALSE, message=FALSE, echo=TRUE, results="hide"}
  # Plot smooth geom UCREATININE vs UALBUMIN, coloring GENDER
  ggplot(data = HANES) + 
  geom_smooth(mapping = aes(x = UCREATININE, y = UALBUMIN, 
                            color = GENDER)) + 
  xlim(c(0,100)) + ylim(c(0,10))
```
...
<br>
<span style="color:blue">**Classwork/Homework**</span>: Try `geom_smooth` on different variables in HANES and MIMIC3 data frames.
...
In fact, we can overlay two geoms on the **same** plot -  
super cool, isn't it? (compare it with `abline()` function):
...
**Note**: This introduces some duplication in our code. 

Imagine if we wanted to change the y-axis to display UACR values instead of UALBUMIN. 

We need to change the variable in two places, and we might forget to update one. We can avoid this type of repetition by passing a set of mappings to the `ggplot()` function. 
...
```{r eval=FALSE, warning=FALSE, message=FALSE, echo=TRUE}
  # Plot smooth and point geom in the same plot 
  ggplot(data = HANES, 
         mapping = aes(x = UCREATININE, y = UALBUMIN, color=GENDER)) + 
  xlim(c(0,100)) + ylim(c(0,10)) +
  geom_smooth() + 
  geom_point()
```
...
**Note**: One can use this idea for plotting _different_ data to different layers. 

For example, the geom `geom_point` can be specified to select DX_DBTS variable consisting only diabetic people, for instance.
...
1. [R for Data Science - Explore Part](http://r4ds.had.co.nz/exploratory-data-analysis.html)
2. [Graphics for communication](http://r4ds.had.co.nz/graphics-for-communication.html#fig:themes)
3. [Graphs with ggplot2 cookbook](http://www.cookbook-r.com/Graphs/)

...
```{r eval=TRUE, message=FALSE}
# Make our data frame - note - merged data has 52 patients and 6531 genes
df <- data.frame(matrix(ncol = 6533, nrow = 52))
...
expression <- as.numeric(t(merged.data[c(1:6531),c(2:53)]))
df[c(1:52),c(3:6533)] <- expression
```
...
Thus, we get an error. All `dplyr()` functions require unique names for the columns. We will see the below tutorial how this situation can be handled. But first, we need to sort this out - we will see about this in detail later...

```{r eval=TRUE, message=FALSE, warning=FALSE}
# Collect only the expression values and official gene symbol
merged.data.official <- merged.data[,c(2:54)]
# Group by official gene name taking the average
merged.data.averaged <- merged.data.official %>% group_by(SYMBOL) %>% summarise_all(funs(mean))
...
```{r eval=FALSE, message=FALSE, warning=FALSE, echo=TRUE}
  ...
  filter(df, Group == 'Luminal', FGFR4 < 5)
```

...
```{r eval=TRUE, message=FALSE, warning=FALSE}
  ...
  # Make a ggplot
  ggplot(data = df) + 
  geom_point(mapping = aes(x = log(TP53), y = log(STAT3)))
```
...
```{r eval=TRUE, message=FALSE, warning=FALSE}
  ...
  ggplot(data = df) + 
  geom_point(mapping = aes(x = log(TP53), y = log(FGFR4), color=Group))
```
...
In the above example, we mapped `Group` to the color aesthetic, but we could have mapped Age-group/Chemo variables to the size aesthetic in the same way. In this case, the exact size of each point would reveal its Age-group affiliation. We get a warning here, because mapping an unordered variable (class) to an ordered aesthetic (size) is not a good idea.

...
```{r eval=TRUE, message=FALSE}
  ...
  imported.RNA.seq.7000 <- read.csv(text=getURL(URL), sep="\t", header = TRUE, stringsAsFactors = FALSE, row.names = 1)
  # Get the gene names
  gene.names <- row.names(imported.RNA.seq.7000)
  ...
  expression <- as.numeric(t(imported.RNA.seq.7000[c(2:6917),c(1:52)]))
  # Get the group values
  group <- t(imported.RNA.seq.7000[1,])

  # Make our data frame - the first column is the patient id
  df <- data.frame(matrix(ncol = 6918, nrow = 52))
  df[,1] <- row.names(group)
  # The columns are the gene names with PID and Group
  colnames(df) <- column.names
  df[,2] <- group[,1]
  # The rest of the matrices are expression values
  df[c(1:52),c(3:6918)] <- expression
```
...
We can now use these official symbol for pathway analysis etc.
...
```{r eval=TRUE, message=FALSE, warning=FALSE}
  ...
  # Make a ggplot
  ggplot(data = df) + 
  geom_point(mapping = aes(x = log(ENSG00000001617), y = log(ENSG00000004897)))
```
...
3. What does the ENSG00000001617 variable describe? 
4. Make a scatterplot of ENSG00000002933 vs ENSG00000002919.
...
We can convey information about the data by mapping the aesthetics in the plot to the variables in df dataset. For example, we can map the colors of our points to the ENSG00000002933 variable to reveal the group status of each person.

```{r eval=TRUE, message=FALSE, warning=FALSE}
  # Make a ggplot with asthetic color for the variable DX_DBTS
  ggplot(data = df) + 
  geom_point(mapping = aes(x = log(ENSG00000001617), y = log(ENSG00000004897), color=Group))
```

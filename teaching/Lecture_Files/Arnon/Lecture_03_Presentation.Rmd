...
### R fundamentals 02: Advanced data types and graphics (`r format(Sys.Date(), "%b %d, %y")`)
...
<br>
An easy/fast way to generate a simple factor with given number of repetitions is by the function `gl()`
```{r, collapse=TRUE}
z <- gl(3, 2, length = 12)
...
<br>
The main difference in subsetting a data.frame vesus matrix is when you specify a single number as index within `[]`. For matrices you get **an element** corresponding to the linear index but for data frame we'll get the **column vector** corresponding to the index. 
...
Adding a column is super easy. Since data frames are list of vectors one can just append a vector to the list. 
...
##### Constructing a data.frame based on selected factors 

`expand.grid()` is one of the most useful, less known functions in R. Allows you to create a Data Frame from all combinations of factor variables. For example:
```{r}
expand.grid(height = seq(60, 70, 5), weight = seq(100, 200, 50),
            sex = c("Male","Female"))
```

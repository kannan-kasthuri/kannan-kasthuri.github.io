...
<iframe width="560" height="315" src="https://www.youtube.com/embed/AUs0X_pBEwo" frameborder="0" allowfullscreen></iframe>
...
Hemoglobin test (or A1C test) is becoming a common test to identify diabetes in people. This test provides information about a person's average levels of blood glucose, also called blood sugar, over the past 3 months. If a person has A1C levels greater than or equal to 6.5% then they are diagnosed diabetic. Also, older adults are at high risk for developing diabetes due to the combined effects of increasing insulin resistance and impaired pancreatic islet function with aging. 

Suppose we want to estimate the probability of a person being diabetic. This probability is conditional on either $A1C \geq 6.5$ or _being old_. If being old is defined as $60+$ years. From the laws of probability we have: 

$$
P(Diab \mid A1C \geq 6.5 \cup\ age \geq 60) = \frac{P([Diab \ \cap \ A1C \geq 6.5] \ \cup \ [Diab \ \cap \ age \geq 60] )}{P(A1C \geq 6.5 \cup\ age \geq 60)} \\
$$

If we define $P(Diab \mid A1C \geq 6.5) = \theta_{A1C}$ and $P(Diab \mid old) = \theta_{Old}$ and

$$
  P(Diab) = \begin{cases}
  p & if \ A1C < 6.5\% \ \&  \ age < 60 \\
  \theta_{A1C} & if \ A1C \geq 6.5\% \ \&  \ age < 60 \\
  \theta_{Old} & if \ A1C \leq 6.5\% \ \&  \ age \geq 60 \\
  \theta_{A1C}+\theta_{Old}-\theta_{A1C}*\theta_{Old} & if \ A1C \geq 6.5\% \ \& \ age \geq 60 \end{cases}
$$

The last equation comes from the 
